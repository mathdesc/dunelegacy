...
Pakfile::Pakfile(std::string pakfilename, bool write)
{
	...
	if(write == false) {
		...
		// Open for writing
		if( (fPakFile = SDL_RWFromFile(filename.c_str(), "a+b")) == NULL) {
			throw std::invalid_argument("Pakfile::Pakfile(): Cannot open " + pakfilename + "!");
		}

		// XXX Should be doing a readindex
...
int Pakfile::WriteFile(SDL_RWops *pRWop, const void *ptr, int size, int n) {
	...
	uint32_t writestartoffset = pPakfile->fileEntries[pRWopData->fileIndex].startOffset + pRWopData->fileOffset;

	if(writestartoffset > pPakfile->fileEntries[pRWopData->fileIndex].endOffset) {
		return -1;
	}


	if(writestartoffset + bytes2write > pPakfile->fileEntries[pRWopData->fileIndex].endOffset) {
			bytes2write = pPakfile->fileEntries[pRWopData->fileIndex].endOffset + 1 - writestartoffset;
			// round to last full block
			bytes2write /= size;
			bytes2write *= size;
			if(bytes2write == 0) {
	...
	if(SDL_RWseek(pPakfile->fPakFile,writestartoffset,SEEK_SET) < 0) {
		return -1;
	}

	if(SDL_RWwrite(pPakfile->fPakFile,ptr,bytes2write,1) != 1) {
		return -1;
	}

	pRWopData->fileOffset += bytes2write;
	return bytes2write/size;
